...
def janela_financeiro():
    # Cria a nova janela para o financeiro
    janela_fin = tk.Toplevel(janela_menu)
    janela_fin.title("Financeiro")
    janela_fin.geometry("600x400")

    tk.Label(janela_fin, text="Financeiro", font=("Arial", 14)).pack(pady=10)

    # Função para calcular os valores financeiros
    def calcular_financeiro():
        try:
            # Cálculo do Lucro Bruto (soma dos preços das vendas)
            cursor.execute('SELECT SUM(Valor_total) FROM vendas')
            resultado_lucro_bruto = cursor.fetchone()
            lucro_bruto = resultado_lucro_bruto[0] if resultado_lucro_bruto[0] is not None else 0.0

            # Cálculo das Despesas (quantidade de cada produto vendido multiplicado pelo valor no estoque)
            comando_despesas = '''
                SELECT SUM(produtos.Preco_custo * vendas.Quantidade_vendida)
                FROM vendas
                INNER JOIN produtos ON vendas.ID_produto = produtos.ID_produto
            '''
            cursor.execute(comando_despesas)
            resultado_despesas = cursor.fetchone()
            despesas = resultado_despesas[0] if resultado_despesas[0] is not None else 0.0

            # Cálculo do Lucro Líquido (Lucro Bruto - Despesas)
            ...
            messagebox.showerror("Erro", f"Ocorreu um erro ao calcular os valores financeiros: {e}")

    # Botão para calcular e exibir os resultados financeiros
    tk.Button(janela_fin, text="Calcular Financeiro", font=("Arial", 12), command=calcular_financeiro).pack(pady=10)

    # Label para exibir os resultados financeiros
    resultado_label = tk.Label(janela_fin, text="", font=("Arial", 12), justify=tk.LEFT)
